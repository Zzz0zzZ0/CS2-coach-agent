...
A real 14-round Demo completed browser upload → Celery → Milvus / Graph → Qwen → Verifier, using 992 model tokens. A second task exposed an asynchronous TLS-client lifecycle defect; the fix passes offline and local HTTPS regressions. The production budget remains stopped after that failure, so a post-fix remote rerun is still pending. Task URLs now restore reports across refreshes. See [live acceptance and limits](docs/LIVE_E2E_V1.md).

The historical data rebuild is live: 20 series, 49 maps and 1,019 regulation/overtime rounds, with four pre-match knife rounds removed. Graph and Milvus evidence, all 56 player profiles and 5,308 silver labels agree; 262 offline tests pass. Production retrieval after the evidence-identity fix passes 50/50 development queries in Vector, Graph and Hybrid modes; the previously observed holdout scores are 28/30, 30/30 and 30/30. See [regression repair](docs/VECTOR_EVIDENCE_IDENTITY_V3.md). Old snapshots remain available for rollback. The corrected 16-query retrieval experiment uses AI-assisted development labels, not independent human gold or unseen generalization evidence. See [rebuild and refreeze](docs/HISTORICAL_DATA_REBUILD_V2.md).
