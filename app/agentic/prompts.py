...
GraphRAG 战术标签的证据等级必须原样保留：event_fact 是事件事实，deterministic_rule 是确定性规则结果，weak_rule（尤其 EXECUTE_CANDIDATE）只是候选序列，不能写成已确认战术。
...
GraphRAG 中 label_source=weak_rule 的 EXECUTE_CANDIDATE 只能称为“疑似爆弹/进攻序列”，不得当作人工确认战术；确定性规则也只能证明规则条件成立，不证明战术意图或因果。
