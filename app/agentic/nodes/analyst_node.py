...
def render_analyst_report(match: dict, metrics: dict) -> str:
    score = "：".join(
    ...
    report = (
        "**比赛概况**\n"
        f"- 地图 {match.get('map_name', 'Unknown')}，共 "
...
def create_analyst_node(llm):
    async def node_analyst(state: GraphState) -> dict:
        ...
        return {"analyst_report": render_analyst_report(state.get("match", {}), state.get("metrics", {}))}
    return node_analyst
