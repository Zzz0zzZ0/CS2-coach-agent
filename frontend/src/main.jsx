...
function App() {
  ...
  async function handleSearch(event) {
    ...
    try {
      ...
      setRelationStatus(response.relation ? { ...response.relation, message: response.message } : null);
...
function GraphCanvas({ positions, edges }) {
  const nodes = Object.values(positions);
  const height = Math.max(320, ...nodes.map(({ y }) => y + 38));
  return <div className="graph-canvas"><div className="graph-scroll" role="region" aria-label="可滚动的战术关系图" tabIndex={0}><svg viewBox={`0 0 900 ${height}`} role="img" aria-label="CS2 tactical graph"><g className="edges">{edges.map((edge, index) => { const from = positions[edge.source]; const to = positions[edge.target]; return from && to ? <line key={index} x1={from.x} y1={from.y} x2={to.x} y2={to.y} /> : null; })}</g><g>{nodes.map(({ x, y, node }) => <g className={`graph-node node-${node.type}`} key={node.id} transform={`translate(${x}, ${y})`}><circle r="17" /><text x="25" y="4">{node.label?.slice(0, 24)}</text><title>{node.id}</title></g>)}</g></svg></div><div className="legend"><span><i className="legend-match" />match/map</span><span><i className="legend-round" />round</span><span><i className="legend-tactic" />tactical sequence</span><span><i className="legend-event" />event</span></div></div>;
}
