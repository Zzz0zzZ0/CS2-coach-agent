...
当前安排（2026-09-07）：独立人工质量盲评延期，作为后续可选研究保留，不阻塞近期开发、验收或展示材料交付。现有人工评分人数仍为 0，质量指标保持 null，不以 AI 审核替代真人盲评。不为等待评审重复生成模型结果或增加调用；重新安排评审时，再冻结修正后的共同事实、模型选择与匿名报告。旧评审包含已知口径缺陷，仅作历史记录，不能直接发给评审者。下文保留原实验协议和首次运行记录。
...
125 项离线测试通过，包含调用前预算限制、失败立即停止、输入变更拒绝、匿名包与身份映射分离、空白评分拒绝出分，以及按比赛而非按地图加权的手算用例。远端 [CI 验证](https://github.com/Zzz0zzZ0/CS2-coach-agent/actions/runs/34043675635) 通过；冻结工具通过不等于模型有增益。


## 首轮运行记录（2026-09-06）

后续 AI 事实审核发现旧包的攻守分母和己方受闪文案存在问题，已在新版修复。旧包作为原始记录保留；下一轮人工评分应重新冻结共同事实和报告。此次 AI 审核未填写人工表、未产生模型增益分数，见 [事实审核](COACH_FACT_AUDIT.md)。

输入和协议于 `75c68e2` 推送后执行。6 图 / 5 个比赛组全部获得有效模型工具选择，共 6 次调用，无重试或规则降级；提供商报告本轮实际用量 4,939 token，本地预留 23,577，调用延迟中位数 1,431.635 ms。用量账完整仅指这 6 次请求，不代表账户免费额度余额。

[运行汇总](../datasets/evaluation/coach_blind_v1_run_report.json) 为 `awaiting_human_review`；两份空白表经过评分入口核验后，[评分状态](../datasets/evaluation/coach_blind_v1_pending_review.json) 为 `pending_review`、完整评审人数 0、质量指标 null。6 例的优先级列表均有差异，但差异本身不是质量增益。

匿名页面已通过浏览器排版检查和全部引用目标存在性检查。历史归档的本地压缩包为 `data/evaluation/coach_blind_v1_review.zip`，仅包含：

- `review/review.html`：解压后用浏览器打开，查看 A/B 报告、共同指标与引用。
- `review/packet.json`：冻结的匿名评审内容。
- `review/reviewer_1.json`、`review/reviewer_2.json`：两位独立评审各填一份，保留两个 SHA-256 字段，填写身份标识、日期、真实声明、全部分数、偏好与理由。

收回填写后的两份 JSON 再执行 `score`，输出使用新文件名，例如 `score_reviewed.json`。方法映射与原始调用结果留在本地运行目录，不在评审压缩包中。当前工程流程完成，人工评审与模型增益结论仍待完成。
