...
真实上传链路已在现有 14 回合 Demo 上跑通：浏览器 → Celery → Milvus / Graph → Qwen → Verifier，首次模型用量 992 tokens。连续第二次调用暴露 TLS 事件循环生命周期问题，已修复并通过离线及本地 HTTPS 回归；生产预算因失败保持暂停，尚未进行修复后的远程复测。真实任务支持 URL 恢复与刷新。见 [完整链路验收与限制](docs/LIVE_E2E_V1.md)。

历史刀局污染已完成重建并切换：原有 20 场、49 图现在包含 1,019 个正式回合，图谱、Milvus、56 名选手画像和 5,308 条战术银标已核对一致。262 项离线测试通过。检索正文与中文道具词形修复后，开发集 Vector / Graph / Hybrid 均为 50/50；既有 holdout 为 28/30、30/30、30/30，均属于已观察样本的回归。见 [检索回归修复](docs/VECTOR_EVIDENCE_IDENTITY_V3.md)。原始数据快照与失败结果均保留，详见 [历史修复与重新冻结](docs/HISTORICAL_DATA_REBUILD_V2.md)。
