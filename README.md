...
历史刀局污染已完成重建并切换：原有 20 场、49 图现在包含 1,019 个正式回合，图谱、Milvus、56 名选手画像和 5,308 条战术银标已核对一致。269 项离线测试通过。检索正文与中文道具词形修复后，开发集 Vector / Graph / Hybrid 均为 50/50；既有 holdout 为 28/30、30/30、30/30，均属于已观察样本的回归。见 [检索回归修复](docs/VECTOR_EVIDENCE_IDENTITY_V3.md)。原始数据快照与失败结果均保留，详见 [历史修复与重新冻结](docs/HISTORICAL_DATA_REBUILD_V2.md)。
